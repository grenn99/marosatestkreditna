...
function handleWelcomeEmail(data) {
  try {
    ...
    if (data.body && typeof data.body === 'object') {
      htmlContent = data.body.html || '';
      textContent = data.body.text || '';
      if (data.body.isWelcome) { // Assuming a similar flag might exist or for clarity
          Logger.log("Welcome email body successfully processed from object.");
      }
    } else if (typeof data.body === 'string') {
      ...
      try {
        ...
        htmlContent = bodyData.html || '';
        textContent = bodyData.text || '';
        Logger.log("Welcome email body successfully processed from parsed string.");
      } catch (parseError) {
      ...
      Logger.log("Warning: data.body for welcome is not an object or string, or is missing. Body received: " + JSON.stringify(data.body));
      // Set to empty or default if body is not in expected format
      htmlContent = '';
      textContent = '';
    }
