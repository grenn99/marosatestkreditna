...
const sensitivePatterns = [
  { pattern: /^sk_(test|live)_[A-Za-z0-9]+$/, name: 'Secret API key' },
  { pattern: /postgres(ql)?:\/\/[^:]+:[^@]+@.+/, name: 'Database connection string' }
];

// Allowed patterns for VITE_ variables (public keys that are meant to be exposed)
const allowedPublicPatterns = [
  { pattern: /^pk_(test|live)_[A-Za-z0-9]+$/, name: 'Public API key' },
  { pattern: /^eyJ[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/, name: 'JWT token' }
];

// Whitelist of VITE_ variables that are allowed to contain sensitive-looking patterns
const whitelistedViteVars = [
  'VITE_SUPABASE_ANON_KEY',
  'VITE_STRIPE_PUBLISHABLE_KEY'
];
...
const viteVarsWithSensitivePatterns = Object.keys(env)
  ...
  .filter(key => !whitelistedViteVars.includes(key)) // Skip whitelisted variables
  .filter(key => {
    const value = env[key];
