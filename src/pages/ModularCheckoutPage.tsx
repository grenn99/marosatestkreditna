 ...
 */
export const ModularCheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        if (gifts.length > 0) {
          // Process gift items here if needed
        }
        ...
        <>
          ...
          {checkoutStep === 'selection' && (
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h2 className="text-xl font-semibold mb-4">{t('checkout.chooseCheckoutMethod', 'Izberite način nakupa')}</h2>
              
              <div className="space-y-4">
                {/* Guest Checkout */}
                <button
                  onClick={() => handleCheckoutSelection('guest_form')}
                  className="w-full p-4 border border-gray-300 rounded-md hover:bg-gray-50 flex justify-between items-center"
                >
                  <span className="font-medium">{t('checkout.guestCheckout', 'Nadaljuj kot gost')}</span>
                  <span className="text-gray-500">→</span>
                </button>
                
                {/* Login */}
                <button
                  onClick={() => handleCheckoutSelection('auth_form')}
                  className="w-full p-4 border border-gray-300 rounded-md hover:bg-gray-50 flex justify-between items-center"
                >
                  <span className="font-medium">{t('checkout.loginCheckout', 'Prijava')}</span>
                  <span className="text-gray-500">→</span>
                </button>
              </div>
            </div>
...
export default ModularCheckoutPage;
